...
test("搜索只保留快速与彻底两种策略，旧宠物模式映射为彻底搜刮", () => {
  ...
  assert.equal(quick.lootMin, 2);
  assert.equal(quick.lootMax, 3);
  assert.equal(quick.supplyCost, 0);
  assert.equal(thorough.lootMin, 3);
  assert.equal(thorough.lootMax, 4);
...
test("每个搜索地点只结算一次，危险仓库提供更多候选且彻底搜刮提高品质", () => {
  ...
  assert.equal(quickSearch.backpack.length, 3);
  assert.equal(thoroughSearch.backpack.length, 3);
  assert.equal(thoroughCache.backpack.length, 4);
  assert.ok(
...
test("满背包的新战利品必须由玩家显式选择替换或离开", () => {
  ...
  assert.equal(overflow.kept, false);
  ...
  assert.equal(run.resolveLootChoice("replace", "b").success, true);
  ...
  assert.equal(lowValue.pending, true);
  assert.equal(run.resolveLootChoice("leave").discarded.id, "scrap");
});
...
test("活动局可保存恢复随机序列，待选物品及显式取舍队列完整保留", () => {
  ...
  assert.equal(restored.getState().lootOverflowQueue.length, 1);
  assert.equal(restored.resolveLootChoice("discard").success, true);
  assert.equal(restored.getState().pendingLootChoice.id, "legacy-queued");
  assert.equal(restored.resolveLootChoice("leave").success, true);
  assert.equal(restored.getState().pendingLootChoice, null);
});

test("主动放弃不保留战斗收益，但保险物仍可带回", () => {
  ...
  assert.equal(abandoned.coins, 0);
  assert.equal(abandoned.exp, 0);
