...
test("探索阶段敌人持续感知与攻击，搜索会被真实受击中断", () => {
  const { combat } = createHarness();
  combat.runSystem.phase = "search";
  combat.currentEncounter = null;
  combat.runSystem.activeSearch = {
    nodeId: combat.runSystem.currentNode?.id || "search-node",
    elapsedMs: 0,
    durationMs: 5000,
  };
  const hero = combat.getHeroCenter();
  const monster = combat.spawnMonster({
    templateId: "slime",
    ambient: true,
    spawnPosition: { x: hero.x + 12, y: hero.y },
  }, 1);
  monster.x = hero.x + 12;
  monster.y = hero.y - monster.height / 2;
  ...
  assert.ok(combat.runHp < hpBefore, "非 combat phase 的近身敌人仍应持续攻击");
  assert.equal(combat.runSystem.activeSearch, null, "搜索必须被敌方伤害中断");
  assert.equal(combat.getBattleState().raidThreatActive, true);
  assert.equal(combat.getBattleState().actions.canFire, true);
});

test("脱战返回 route 且不结算节点，残血敌人留在地图并在重返时复用", () => {
  const { combat } = createHarness();
  const node = combat.runSystem.routeChoices.find(item => !["search", "cache", "camp"].includes(item.type));
  assert.ok(node);
  combat.runSystem.currentNode = { ...node };
  combat.runSystem.phase = "combat";
  assert.equal(combat.worldSystem.engageLocation(node.id).success, true);
  combat.beginEncounter({ type: node.type, depth: node.depth, enemyCount: 1, nodeId: node.id });
  combat.encounterQueue = [];
  const encounterId = combat.currentEncounter.encounterId;
  const hero = combat.getHeroCenter();
  const survivor = combat.spawnMonster({
    templateId: "slime",
    encounterId,
    homeNodeId: node.id,
    spawnPosition: { x: hero.x + 800, y: hero.y },
  }, 1);
  survivor.hp = Math.max(1, survivor.maxHp - 5);
  survivor.homeX = combat.getEntityCenter(survivor).x;
  survivor.homeY = combat.getEntityCenter(survivor).y;
  survivor.aiState = "search-return";
  survivor.searchTimer = 0;
  survivor.lastKnownHero = null;
  const pendingBefore = { ...combat.runSystem.pendingRewards };

  combat.update(16);

  assert.equal(combat.runSystem.phase, "route");
  assert.equal(combat.runSystem.currentNode.id, node.id, "脱战不能完成或清空当前节点");
  assert.equal(combat.runSystem.completedNodeIds.includes(node.id), false);
  assert.deepEqual(combat.runSystem.pendingRewards, pendingBefore, "未击杀敌人时脱战不能结算奖励");
  assert.equal(combat.monsters.includes(survivor), true);
  assert.equal(survivor.origin, "ambient");
  assert.equal(combat.worldSystem.getLocationByNodeId(node.id).state, "available");
  assert.equal(combat.worldSystem.activeLocationId, null);
  const savedHp = survivor.hp;

  assert.equal(combat.chooseRoute(node.id).success, true);
  assert.equal(combat.monsters.includes(survivor), true);
  assert.equal(survivor.hp, savedHp);
  assert.equal(survivor.encounterId, combat.currentEncounter.encounterId);
  assert.equal(combat.encounterQueue.length, 0, "重返同一地点应复用残敌而非叠加完整新波次");
});

test("空弹时保底近战不会软锁，地图弹药拾取会补入当前武器备弹", () => {
  const { combat } = createHarness();
  combat.runSystem.phase = "route";
  combat.currentEncounter = null;
  combat.ambientPatrolQueue = [];
  const hero = combat.getHeroCenter();
  const monster = addMonster(combat, { x: hero.x + 48, y: hero.y - 15, hp: 100 });
  combat.setAimDirection(1, 0);
  combat.weaponStates.rifle.magazine = 0;
  combat.weaponStates.rifle.reserve = 0;
  const hpBefore = monster.hp;

  const fallback = combat.fireCurrentWeapon();

  assert.equal(fallback.success, true);
  assert.equal(fallback.fallback, true);
  assert.equal(fallback.hit, true);
  assert.ok(monster.hp < hpBefore);
  assert.equal(combat.getWeaponState().fallbackReady, false);

  const pickup = combat.spawnAmmoPickup({
    x: hero.x,
    y: hero.y,
    weaponId: "rifle",
    amount: 12,
  ...
  assert.ok(pickup);
  const collected = combat.collectNearbyAmmoPickups();
  assert.equal(collected.length, 1);
  assert.equal(combat.weaponStates.rifle.reserve, 12);
  assert.equal(combat.getAmmoPickupState().some(item => item.id === pickup.id), false);
});

test("开始节点遭遇不会清空已有环境巡逻敌人", () => {
  const { combat } = createHarness();
  combat.runSystem.phase = "route";
  combat.currentEncounter = null;
  const ambient = combat.spawnMonster({
    templateId: "bat",
    ambient: true,
    spawnPosition: { x: 900, y: 900 },
  }, 1);
  const bullets = combat.bullets;

  combat.beginEncounter({ type: "combat", depth: 1, enemyCount: 1 });

  assert.equal(combat.monsters.includes(ambient), true);
  assert.equal(combat.bullets, bullets, "进入遭遇不能删除世界中已经飞行的弹丸");
  assert.equal(ambient.aiState, "patrol");
});

test("Run 与 World 进入状态保持原子，探索计数由完整热点派生", () => {
  const { combat } = createHarness({ autoStart: false });
  assert.equal(combat.startRun().success, true);
  const state = combat.getBattleState();
  assert.equal(state.hotspotCount, 8);
  assert.equal(state.exploredCount, 0);

  const node = combat.runSystem.routeChoices.find(item => item.type === "combat");
  const location = combat.worldSystem.getLocationByNodeId(node.id);
  location.state = "engaged";
  const blocked = combat.chooseRoute(node.id);
  assert.equal(blocked.success, false);
  assert.equal(combat.runSystem.phase, "route");
  assert.equal(combat.runSystem.currentNode, null);

  location.state = "available";
  combat.runSystem.completedNodeIds = [node.id];
  assert.equal(combat.getBattleState().exploredCount, 1);
});
