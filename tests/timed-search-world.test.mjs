...
test("计时搜索完成前不产生掉落、威胁、伏击或资源消耗", () => {
  ...
  assert.deepEqual(run.searchMetrics, { timeSeconds: 0, suppliesSpent: 0 });
  ...
  assert.equal(run.beginSearch("quick", { containerId: "crate-b" }).success, false, "同一地点不能出现第二次搜索结算");
});
...
test("搜索地点只生成一个交互点，搜索状态可完整保存恢复", () => {
  ...
  assert.equal(engaged.containers.length, 1);
  ...
  assert.equal(began.context.remainingContainerCount, 1);
  assert.equal(began.context.isLastContainer, true);
  ...
  const completed = restored.completeContainerSearch(first.id);
...
test("旧存档中的多容器地点会收敛为一个搜索实体", () => {
  const routes = [createRouteNode("route-search", "search", 1, 0)];
  const world = new ExpeditionWorldSystem();
  world.startRun(routes, { seed: 7789 });
  const engaged = world.engageLocation("route-search");
  const retained = engaged.containers[0];
  const snapshot = world.getRunSaveData();
  snapshot.containers.push({
    ...retained,
    id: `${retained.id}-legacy-extra`,
    x: retained.x + 20,
    state: "available",
  });
  snapshot.nearbyContainerId = `${retained.id}-legacy-extra`;

  const restored = new ExpeditionWorldSystem();
  assert.equal(restored.loadRunSaveData(snapshot).success, true);
  assert.equal(restored.getContainersForLocation("route-search").length, 1);
  assert.ok(restored.getState(retained).nearbyContainer);
});
...
test("活动中的搜索读条可保存并从原进度继续", () => {
  ...
  assert.equal(restored.getState().searchState.remainingMs, 750);
  const completed = restored.updateSearch(750);
  assert.equal(completed.success, true);
  assert.equal(completed.completed, true);
