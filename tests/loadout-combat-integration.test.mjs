...
test("实际远征掉落包含可配装物，并能从仓库带入下一局产生效果", () => {
  ...
  assert.equal(combat.switchWeapon("marksman").success, true, "局内仍可切换已携带的战斗模式");
  assert.equal(combat.getBattleState().weapon.activeWeaponId, "marksman");
  assert.equal(meta.getLoadout().mainWeapon.combatWeaponId, "shotgun", "局内切换不得改写局外配装");
  assert.equal(
