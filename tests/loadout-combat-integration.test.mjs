...
test("实际远征掉落包含可配装物，并能从仓库带入下一局产生效果", () => {
  ...
  assert.equal(combat.switchWeapon("marksman").success, false, "局内必须使用出发时锁定的武器");
  assert.equal(combat.getBattleState().weapon.activeWeaponId, "shotgun");
  assert.equal(meta.setWeaponMode("marksman").success, false, "活动远征期间不能绕过战斗系统改写武器模式");
  assert.equal(meta.getLoadout().mainWeapon.combatWeaponId, "shotgun", "局外配装保持出发时选择");
  assert.equal(
    meta.getState().activeRaid.loadoutSnapshot.mainWeapon.combatWeaponId,
    "shotgun",
    "局内切换不得改写出发时锁定的活动远征快照",
  );
