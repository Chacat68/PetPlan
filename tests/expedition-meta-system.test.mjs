...
test("合约槽严格限制为一条主线和两条支线", () => {
  const meta = new ExpeditionMetaSystem();
  assert.equal(accept(meta, "main-supply-recovery").success, true);
  assert.match(accept(meta, "main-investigate-vault").message, /主线合约槽已满/);
...
test("调查、精英击杀、指定宠物和高威胁撤离事件均可独立推进", () => {
  ...
  const investigateContract = accept(investigation, "main-investigate-vault").contract;
  investigation.recordContractEvent({ type: "location-investigated", locationId: "wrong-place" });
  assert.equal(investigation.getContractState().active[0].progress, 0);
  investigation.recordContractEvent({ type: "location-investigated", locationId: "core-vault" });
...
test("applySettlement 可用 runStats 一次更新精英、探索和撤离合约", () => {
  const meta = new ExpeditionMetaSystem();
  accept(meta, "main-investigate-vault");
  ...
  const result = meta.applySettlement({
    ...
    runStats: {
      eliteKills: 3,
      investigatedLocationIds: ["core-vault"],
      petIds: ["pet-cat"],
    },
  });
